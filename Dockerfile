...
RUN apt-get update && \
    apt-get install -y \
        tzdata openssh-server sudo curl ca-certificates wget vim net-tools \
        supervisor cron unzip iputils-ping telnet git iproute2 nano htop \
        python3.10 python3-pip bash --no-install-recommends && \
    # 清理缓存
    apt-get clean && \
